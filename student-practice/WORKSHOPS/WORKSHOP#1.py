...
# ----> no because the base case is met and the function will stop itself recursively if x == 1. 
...
# ----> to find 5th term on the fibonacci sequence, the number must meet the base case in order
#       to find the term. In order to do so, python will read synchronously meaning it reads top/down, left/right
#       THEREFORE, it will need to solve the recursion tree on the left side until the first recursive function is met
#       THEN, it will solve for the right side of the recursion tree. 